...
export default function Ocr({fileData,setText}) {
    ...
    const [capturedFileId, setCapturedFileId] = React.useState(null)  //id of image file in Capture Service
    ...
    function dataURLToBlob(dataURL) {
        ...
        return new Blob([uint8Array], { type: mimeString, name: "tempName" });
    }
    ...
    useEffect( () => {
        if(ocrResponse == null || ocrResponse.resultItems[0]["files"][0]["name"]=="")
        {
            return
        }
        retrieveFileFromCaptureService()
    } , [ocrResponse] )
  ...
  async function retrieveFileFromCaptureService() {
    ...
    fetch(captureUrl, captureFetchOptions).then( function(response) {
